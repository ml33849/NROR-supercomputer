...
int main() {
    ...
    for (int ii = 0; ii < 2000; ii++)
    {
        for (int i = 0; i < n; i++) {
            ...
            // Paralelizirana notranja zanka, vsak člen v sigma=VSota (​A[i][j]⋅T[j]) je popolnoma neodvisen, in tako je mogoča paralelizacija
            #pragma omp parallel for reduction(+:sigma) //omogoča paralelizacijo notranje zanke
            for (int j = 0; j < n; j++) {
                if (j != i) {
                    sigma += A[i][j] * T[j];
                }
            }
            /*Ta korak mora biti sekvenčen in nemore biti praleliziran, saj so posodobljene vrednosti T[i]T[i] 
            odvisne od že posodobljenih vrednosti T[j]T[j], kjer je j<ij<i.*/
            T[i] = (b[i] - sigma) / A[i][i];
        }
    }
    /*Metoda je delno paralelizirana. Zunanja zanjka ni, notranja pa je.
    Paralelizacija notranje zanke zmanjša čas za izračun, vendar glavni 
    časovni vpliv še zmeraj predstavlja odvisnost med iteracijami zunanje zanke.*/
